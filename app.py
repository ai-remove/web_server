...
def fg_dir(dir_path, username, fold):
    fg_dir = os.path.join(dir_path, 'src', 'user_data', username, 'foreground', fold)
    return fg_dir

def bg_dir(dir_path, username, fold):
    bg_dir = os.path.join(dir_path, 'src', 'user_data', username, 'background', fold)
    return bg_dir

def processed_dir(dir_path, username, fold):
    processed_dir = os.path.join(dir_path, 'src', 'user_data', username, 'processed', fold)
    return processed_dir

#####   File paths   ###### 

def output_path(dir_path, username):
    out_video_path = os.path.join(dir_path, 'src', 'user_data', username, 'processed', 'output', 'output.mp4')
    out_gif_path = os.path.join(dir_path, 'src', 'user_data', username, 'processed', 'output', 'gif_output.gif')
...
def post():
    ...
    string_index = output_gif_path.find("/user_data/")
    output_video_path = output_video_path[string_index:].strip()
    print(output_video_path)
    #output_video_path = 'man3/processed/output/output.mp4'
...
def signup():
    ...
    Path(dir_path+"/src"+"/user_data/"+username).mkdir(parents=True, exist_ok=True)
    Path(dir_path+"/src"+"/user_data/"+username+"/foreground").mkdir(parents=True, exist_ok=True)
    Path(dir_path+"/src"+"/user_data/"+username+"/background").mkdir(parents=True, exist_ok=True)
    Path(dir_path+"/src"+"/user_data/"+username+"/processed").mkdir(parents=True, exist_ok=True)
